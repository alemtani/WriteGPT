...
@works.route('/works/<int:work_id>', methods=['GET'])
@token_auth.login_required
def get_work(work_id):
    return jsonify(db.session.query(Work).get_or_404(work_id).to_dict())

@works.route('/works/<int:work_id>', methods=['PUT'])
@token_auth.login_required
def update_work(work_id):
    work = db.session.query(Work).get_or_404(work_id)
...
@works.route('/works/<int:work_id>', methods=['DELETE'])
@token_auth.login_required
...
@works.route('/works/<int:work_id>/likers', methods=['GET'])
@token_auth.login_required
def get_likers(work_id):
    work = db.session.query(Work).get_or_404(work_id)
    page = request.args.get('page', 1, type=int)
    per_page = min(request.args.get('per_page', 10, type=int), 100)
